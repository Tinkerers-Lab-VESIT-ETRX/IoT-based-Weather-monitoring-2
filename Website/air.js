function myFunction() {
    var selectedDate=document.getElementById("demo4").value;
    console.log(selectedDate);
    const Http = new XMLHttpRequest();
    const url='https://weather-monitoring-12aee-default-rtdb.firebaseio.com/AIR_QUALITY_data.json';
        ...
        if (Http.readyState == 4 && Http.status == 200) {
            ...
            for(i=0;i<resData.length;i++){
                ...
                if(resData[i]['date']==selectedDate){
                    var air1=resData[i]['date'].split('-');
                    var air2=resData[i]['time'].split(':');
                    var x=new Date(air1[0],air1[1]-1,air1[2],air2[0],air2[1]);
                    console.log(x,y);
